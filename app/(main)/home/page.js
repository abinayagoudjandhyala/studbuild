...
export default function HomePage() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (!data.user || !data.user.isOnboarded) { router.push('/onboarding'); return }
      setProfile(data.user)
      const ids = (data.user.following || []).map(f => (typeof f === 'string' ? f : f._id || f))
  ...
  const handlePeopleSearch = (val) => {
    ...
    if (!val.trim()) { setPeopleResults([]); return }
    searchTimeout.current = setTimeout(() => searchPeople(val), 350)
  }
  ...
  const searchPeople = async (q) => {
    ...
    try {
      ...
      const data = await res.json()
      const users = (data.users || []).filter(u => u.clerkId !== user?.id)
  ...
  const toggleFollow = async (targetUser) => {
    ...
    try {
      await fetch('/api/users/' + targetUser.clerkId + '/follow', { method: 'POST' })
    } catch (err) {
      console.error(err)
      // revert on error
      setFollowingIds(prev => {
    ...
    <div style={{ minHeight: '100vh', background: '#06080f', fontFamily: 'var(--font-dm), DM Sans, sans-serif' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '2rem' }}>
        ...
        <div style={{
          ...
          {/* Divider */}
          <div style={{ height: 1, background: 'linear-gradient(90deg,rgba(79,142,247,0.3),rgba(167,139,250,0.2),transparent)', marginBottom: '1.2rem' }} />

          {/* Quick actions */}
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <p style={{ width: '100%', fontSize: '0.72rem', color: '#8896b0', marginBottom: 2, letterSpacing: '0.06em' }}>QUICK ACTIONS</p>
            {[
              ...
              <button key={path} onClick={() => router.push(path)} style={{
                ...
                background: primary ? 'linear-gradient(135deg,#4f8ef7,#7c5cfc)' : 'rgba(255,255,255,0.04)',
                ...
                onMouseEnter={e => { if (!primary) { e.currentTarget.style.borderColor = 'rgba(79,142,247,0.4)'; e.currentTarget.style.color = '#f0f4ff'; e.currentTarget.style.background = 'rgba(79,142,247,0.08)' } }}
                onMouseLeave={e => { if (!primary) { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'; e.currentTarget.style.color = '#8896b0'; e.currentTarget.style.background = 'rgba(255,255,255,0.04)' } }}
              >{label}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{
          ...
          <div>
            ...
            {myProjects.length > 0 && (
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.8rem' }}>
                  <h3 style={{ fontFamily: 'var(--font-syne), Syne, sans-serif', fontSize: '0.88rem', fontWeight: 600 }}>Your Projects</h3>
                  <span onClick={() => router.push('/profile/' + profile?.clerkId)}
                    style={{ color: '#4f8ef7', fontSize: '0.78rem', cursor: 'pointer' }}>View all →</span>
                </div>
                <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 4 }}>
                  {myProjects.slice(0, 5).map(p => (
                    ...
                    >
                      <div style={{
                        height: 70,
                        background: p.coverImage ? 'transparent' : 'linear-gradient(135deg,#1a2a4a,#2a1a4a)',
            ...
            {/* Nav Tiles — Projects (inline) | Blogs | Q&A (navigate) */}
            <div style={{
              display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10,
              marginBottom: '1.2rem', animation: 'fadeUp 0.5s 0.15s ease both'
            }}>
              {/* Projects — active/selected look, no navigation */}
              <div style={{
                background: 'linear-gradient(135deg,#4f8ef7,#7c5cfc)',
              ...
              <div onClick={() => router.push('/blogs')}
                ...
                onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(79,142,247,0.4)'; e.currentTarget.style.background = '#111827' }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = '#1f2d45'; e.currentTarget.style.background = '#0d1120' }}
              >
                <span style={{ fontSize: isMobile ? '1.2rem' : '1.5rem' }}>📝</span>
                <span style={{
                  fontFamily: 'var(--font-syne), Syne, sans-serif',
                  fontSize: '0.88rem', fontWeight: 700, color: '#f0f4ff'
                }}>Blogs</span>
                <span style={{ fontSize: '0.7rem', color: '#4f8ef7' }}>Explore →</span>
              </div>
                ...
                <>
                  ...
                  >
                    ...
                    <div style={{ padding: '1rem 1.5rem', display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <span style={{ fontSize: '0.85rem', color: '#8896b0', flex: 1 }}>{projects[0].author?.firstName} {projects[0].author?.lastName}</span>
          ...
          {!isMobile && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              {/* Find People — search & follow */}
              <div style={{ background: '#0d1120', border: '1px solid #1f2d45', borderRadius: 16, padding: '1.2rem', animation: 'fadeUp 0.5s 0.2s ease both' }}>
                <h3 style={{ fontFamily: 'var(--font-syne), Syne, sans-serif', fontSize: '0.85rem', fontWeight: 600, marginBottom: '0.7rem' }}>
                  🔎 Find people
                </h3>

                {/* Search input */}
                <div style={{ position: 'relative', marginBottom: peopleResults.length || searchingPeople ? '0.7rem' : 0 }}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#4f6070"
                  ...
                  <input
                    ...
                    style={{
                      ...
                      background: '#111827', border: '1px solid #1f2d45',
                      borderRadius: 8, color: '#f0f4ff', fontSize: '0.8rem',
                ...
                {/* Results */}
                {searchingPeople ? (
                  <div style={{ textAlign: 'center', padding: '0.8rem 0', color: '#4f6070', fontSize: '0.78rem' }}>
                    Searching...
                  </div>
                    ...
                    {peopleResults.slice(0, 6).map(u => {
                      ...
                      return (
                        <div key={u._id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <div onClick={() => router.push('/profile/' + u.clerkId)} style={{ cursor: 'pointer', flexShrink: 0 }}>
                            {u.avatar
                              ? <img src={u.avatar} alt="av" style={{ width: 32, height: 32, borderRadius: '50%', objectFit: 'cover' }} />
                              : <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(79,142,247,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.72rem', fontWeight: 600, color: '#4f8ef7' }}>{u.firstName?.[0]}{u.lastName?.[0]}</div>
                            }
                          </div>
                          <div onClick={() => router.push('/profile/' + u.clerkId)} style={{ flex: 1, minWidth: 0, cursor: 'pointer' }}>
                          ...
                          <button onClick={() => toggleFollow(u)} style={{
                            ...
                            border: isFollowing ? '1px solid rgba(79,142,247,0.3)' : '1px solid #1f2d45',
                            background: isFollowing ? 'rgba(79,142,247,0.1)' : 'linear-gradient(135deg,#4f8ef7,#7c5cfc)',
                            color: isFollowing ? '#4f8ef7' : '#fff',
                ...
                {!peopleQuery && (
                  <p style={{ fontSize: '0.74rem', color: '#4f6070', marginTop: peopleResults.length ? 0 : '0.4rem' }}>
                    Search for builders by name to follow them
                  </p>
                )}
              </div>
              ...
              {/* GitHub Stats */}
              {githubStats && (
                <div style={{ background: '#0d1120', border: '1px solid #1f2d45', borderRadius: 16, padding: '1.2rem', animation: 'fadeUp 0.5s 0.25s ease both' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.8rem' }}>
                    <h3 style={{ fontFamily: 'var(--font-syne), Syne, sans-serif', fontSize: '0.85rem', fontWeight: 600 }}>⚡ GitHub</h3>
                    <a href={githubStats.profileUrl} target="_blank" rel="noreferrer" style={{ color: '#4f8ef7', fontSize: '0.72rem', textDecoration: 'none' }}>View →</a>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    {[
                      ...
                      <div key={label} style={{ background: '#111827', borderRadius: 10, padding: '0.65rem', border: '1px solid #1f2d45', textAlign: 'center' }}>
                        <div style={{ fontSize: '0.95rem', marginBottom: 2 }}>{icon}</div>
                        <div style={{ fontFamily: 'var(--font-syne), Syne, sans-serif', fontSize: '1rem', fontWeight: 700, background: 'linear-gradient(120deg,#4f8ef7,#a78bfa)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{value}</div>
                        <div style={{ fontSize: '0.65rem', color: '#8896b0' }}>{label}</div>
                      </div>
                    ))}
                  </div>
                    ...
                    <div style={{ marginTop: '0.8rem' }}>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                        {githubStats.languages.slice(0, 4).map(lang => (
                          <span key={lang} style={{ padding: '0.15rem 0.55rem', borderRadius: 999, fontSize: '0.7rem', background: '#111827', color: '#f0f4ff', border: '1px solid #1f2d45' }}>{lang}</span>
                        ))}
                      </div>
                    </div>
                  )}
              ...
              {profile && (() => {
                ...
                return (
                  <div style={{ background: '#0d1120', border: '1px solid #1f2d45', borderRadius: 16, padding: '1.2rem', animation: 'fadeUp 0.5s 0.3s ease both' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                      <h3 style={{ fontFamily: 'var(--font-syne), Syne, sans-serif', fontSize: '0.85rem', fontWeight: 600 }}>Profile Strength</h3>
                    ...
                    <div style={{ background: '#1f2d45', borderRadius: 999, height: 5, marginBottom: '0.8rem' }}>
                      <div style={{ background: 'linear-gradient(90deg,#4f8ef7,#7c5cfc)', borderRadius: 999, height: 5, width: pct + '%', transition: 'width 0.5s ease' }} />
                    </div>
